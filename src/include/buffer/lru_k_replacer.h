...
class LRUKReplacer {
 ...
 private:
  ...
  size_t curr_size_{0};                        // 当前帧的容量
  size_t replacer_size_;                       // LRU-K可以替换的帧的数量
  size_t k_;                                   // k值
  std::mutex latch_;                           // 互斥锁
  std::vector<bool> is_accessible_;            // 向量数组表示哪一些帧可以被驱逐出去
  std::unordered_map<int, size_t> use_count_;  // 每个帧出现频次的哈希表
  // 历史队列
